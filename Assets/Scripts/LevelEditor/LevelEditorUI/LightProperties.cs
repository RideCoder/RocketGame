...
public class LightProperties : MonoBehaviour
{
    ...
    public void ChangeRadius(float radius)
    {
        foreach (GameObject go in LevelEditorController.targetObjects)
        {
            ...
            if (!go.TryGetComponent(out LightObject levelObject))
                continue;

            levelObject.lightComponent.range = radius * 10f;
        }
    }
    ...
    private void ApplyColorHSV(float? h, float? s, float? v)
    {
        foreach (GameObject go in LevelEditorController.targetObjects)
        {
            ...
            Color current = levelObject.lightComponent.color;
            ...
            levelObject.lightComponent.color = Color.HSVToRGB(nh, ns, nv);
        }
    }
    ...
    public void ChangeIntensity(float intensity)
    {
        foreach (GameObject go in LevelEditorController.targetObjects)
        {
            ...
            levelObject.lightComponent.intensity = intensity;
        }
    }
}
